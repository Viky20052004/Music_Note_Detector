...
def firstwin():
    ...
    fir.geometry('2000x2000')
    bg = tk.PhotoImage(file='C:/Users/User/PycharmProjects/arduino/2.png')
    l17 = tk.Label(fir, image=bg)
    l17.pack()
    ex=tk.Button(fir,text='Existing user',font=(25),command=exist)
    ex.place(x=550,y=300)
    new=tk.Button(fir,text='New user',font=(40),command=newuser)
...
def mainprogram():
    ...
    *****'''
    ...
    c0=tk.Label(detect,text="Play your note (atleast for 3 seconds)",font=(20))
    c0.place(x=450,y=250)
    time.sleep(1)
    #o=tk.Label(detect,text='Press OK')
    #o1=tk.Button(detect,text='  OK  ',font=(20),command=output)
    '''
    *****
...
def output():
    ...
    ******'''
    output=tk.Tk()
    output.title('OUTPUT')
    output.geometry('2000x2000')
    time.sleep(1)
    #total = 0
    #print("Unable to print the result, an error occurred! please try again !")
    out=tk.Label(output,text='The frequency of the note you played is approximately 130.8',font=(20))
    time.sleep(1)
    out1=tk.Label(output,text='I think you played D#',font=(20))
    out.place(x=400,y=250)
    time.sleep(1)
    out1.place(x=400,y=300)
